...
const Contact = () => {
  ...
  // Send the contact form to Shashank's Gmail via FormSubmit.
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('https://formsubmit.co/ajax/shashank2009s24@gmail.com', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...
          permission: formData.permission ? 'Yes' : 'No'
        })
      });

      if (!response.ok) throw new Error('Failed to send message');

      alert(`Thanks ${formData.firstName}! Your message has been sent successfully.`);
      setFormData({ firstName: '', lastName: '', email: '', message: '', permission: false });
    } catch (error) {
      console.error('Contact form submission failed:', error);
      alert('Sorry, your message could not be sent right now. Please try again or email me directly at shashank2009s24@gmail.com.');
    }
  };
